import { IEditorAdapter, DetectionResult, EditorCapabilities } from './types';
...
export class EditorRegistry {
  ...
  async getBestAdapter(): Promise<IEditorAdapter | null> {
    ...
    const installed = Array.from(this.adapters.values()).filter(
      (adapter) => available.get(adapter.editorId)?.isInstalled
    );
    ...
    installed.sort(
      (a, b) =>
        syncLevelPriority[b.capabilities.syncLevel] - syncLevelPriority[a.capabilities.syncLevel]
    );
